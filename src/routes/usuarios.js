...
router.get('/carrito', verifyToken, (req, res) => {
  db.query(
    `SELECT ac.id, ac.producto_id, p.nombre, p.precio_calculado, ac.cantidad, 
  ...
  db.getConnection((err, connection) => {
    ...
    connection.beginTransaction((err) => {
      ...
      connection.query(
       ...
       const valoresProductos = productos.map(p => [venta_id, p.producto_nombre, p.cantidad, p.precio_calculado]);
